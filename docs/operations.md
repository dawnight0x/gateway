...
收到退出、SIGTERM 或托盘重启请求后，网关先进入 `draining`：`/health` 继续返回 `200`，`/ready` 返回 `503 shutdown_in_progress`，新的生成请求返回 `503 gateway_draining` 和 `Retry-After`。经过 `server.drain_grace_seconds` 后停止监听，并最多等待 `server.shutdown_timeout_seconds` 让进行中的请求结束。

停机等待时间建议不小于 `routing.timeout_seconds`。持续时间不可预测的长流式业务应在升级窗口前停止接入，并通过 `gateway_in_flight_requests` 确认活跃请求已经归零。
