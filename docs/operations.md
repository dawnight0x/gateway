...
`/metrics` 提供 Key 数量、冷却状态、今日请求/Token、请求日志丢弃、当前并发、上游尝试、重试和过载拒绝指标。今日请求/Token 使用独立日聚合表，即使关闭详细请求日志也会保留。请求可通过响应头 `X-Gateway-Request-ID` 与后台请求日志关联。后台日志同时记录逻辑路由、实际上游模型和尝试次数，可用于还原模型切换顺序。
...
自动模型发现由 `model_discovery` 配置控制，只对启用且拥有可用 Key 的 Provider 执行模型列表读取。刷新失败或返回空列表不会删除上次成功库存；后台会保留最后成功时间并显示本次错误。若上游模型已经下线，可在逻辑路由中禁用对应目标，等待冷却结束后再恢复。

逻辑模型按“模型优先级、Provider 优先级、Key 顺序”生成候选。`model_unavailable` 只冷却对应 Provider/模型组合；401/403、429、网络和服务错误按现有 Key 健康策略处理。单次请求最多尝试 `routing.retry_per_request` 个候选，因此该值应覆盖希望参与一次故障切换的候选数量，同时保留防止重复计费的上限。
