...
自动模型发现由 `model_discovery` 配置控制，只对启用且拥有可用 Key 的 Provider 执行模型列表读取。发现任务会跟随 Gemini、Anthropic 等上游的分页游标，读取同 Provider 的全部启用 Key，并把各 Key 的成功结果合并显示。单个 Key 刷新失败或返回空列表不会删除该 Key 上次成功库存；后台会保留最后成功时间并显示本次错误。Provider 卡片支持搜索、全选、仅选旗舰推荐和清空模型；开启模型白名单后，仅选中模型参与直连和逻辑路由，新发现模型不会自动加入。空白名单表示该 Provider 暂停参与模型调度。

逻辑模型按“模型优先级、Provider 优先级、Key 顺序”生成候选，路由层会自行排序，不依赖数据库返回顺序。模型错误只冷却对应 Provider/Key/模型组合；同一次请求内，明确的模型不存在或不支持会过滤该 Provider 的其他 Key/同模型，模型权限错误仍只过滤失败 Key。401/403、429、网络和服务错误按现有 Key 健康策略处理。显式逻辑路由会完整遍历确定性失败的候选；`routing.retry_per_request` 限制普通 Key 路由。显式逻辑路由默认不会因高优先级候选达到并发上限而降级；需要容量溢出时开启 `routing.fallback_on_busy`。未配置逻辑路由的原有 Key 容量切换保持不变。后台模型路由页面只显示尚未到期的冷却状态，并可手动重置目标下全部 Key 的模型健康状态。
