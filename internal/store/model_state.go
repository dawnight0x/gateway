...
func (s *Store) ListProviderModelStates(ctx context.Context) ([]model.ProviderModelState, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT provider_id,model_id,consecutive_failures,cooldown_until,last_error,last_status_code,
			success_count,failure_count,last_used_at
		FROM provider_model_state ORDER BY provider_id,model_id
	`)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&item.ProviderID, &item.ModelID, &item.ConsecutiveFailures, &cooldown, &item.LastError,
...
func (s *Store) RecordProviderModelSuccess(ctx context.Context, providerID, modelID string) error {
...
func (s *Store) RecordProviderModelFailure(ctx context.Context, providerID, modelID string, status *int, message string, policy FailurePolicy) error {
	...
	if policy.ForceCooldown || failures >= policy.Threshold {
		...
		if _, err := tx.ExecContext(ctx, `
			UPDATE provider_model_state SET cooldown_until=? WHERE provider_id=? AND model_id=?
...
func (s *Store) ResetProviderModelState(ctx context.Context, providerID, modelID string) error {
	_, err := s.db.ExecContext(ctx, `
		UPDATE provider_model_state
		SET consecutive_failures=0,cooldown_until=NULL,last_error='',last_status_code=NULL
		WHERE provider_id=? AND model_id=?
